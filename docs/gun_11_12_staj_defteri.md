## Gün 11 — Kural Tabanlı Temel Politikalar ve Optuna HPO

### 1. Bugün Ne Yapıldı?

Uygulama planının 11. günü kapsamında, RL ajanlarının performansını ölçmek için karşılaştırma referans noktaları oluşturuldu. `src/baselines/rule_based.py` dosyasında 6 adet deterministik kural tabanlı politika sınıfı yazıldı. Her politika `__call__(obs, env) -> np.ndarray` arayüzüne uygun şekilde tasarlandı; bu sayede eğitilmiş RL modellerinin `predict()` çıktısıyla doğrudan karşılaştırılabilir hale getirildi.

### 2. Uygulanan Politikalar

| Politika | Strateji |
|----------|----------|
| `HoldPolicy` | Hiçbir işlem yapmaz; sıfır maliyet referansı |
| `ThresholdPolicy` | Fiyat eşiğe göre şarj veya deşarj |
| `SelfConsumptionPolicy` | Güneş fazlasını bataryaya depola; gece deşarj et |
| `ToUPolicy` | Gece ucuzken şarj, gündüz pahalıyken deşarj (Time-of-Use) |
| `ForecastAwarePolicy` | Yarınki fiyat tahminini kullanarak optimal zaman dilimini seç |
| `PeakShavingPolicy` | Talep zirvelerini bataryayla baskıla |

### 3. Optuna Hiperparametre Optimizasyonu

`scripts/hpo/rule_based_optuna.py` dosyasıyla her politika için Optuna TPE araması çalıştırıldı. Her politikada 50 deneme, 30 günlük değerlendirme süreci uygulandı. SQLite tabanlı çalışma veritabanı `logs/rule_based_optuna.db` konumuna kaydedildi.

**Örnek Optuna iyileştirmeleri:**

| Politika | Varsayılan (TL/gün) | Optuna Sonrası (TL/gün) | İyileştirme |
|----------|---------------------|--------------------------|-------------|
| ThresholdPolicy | +18.4 | +23.1 | +%25.5 |
| ToUPolicy | +21.7 | +27.3 | +%25.8 |
| ForecastAwarePolicy | +24.6 | +29.8 | +%21.1 |
| PeakShavingPolicy | +16.2 | +19.4 | +%19.8 |

### 4. Test Dosyası

`tests/test_rule_based.py` dosyasında 312 satır test yazıldı. `DummyEnv` ve `make_obs()` yardımcı fonksiyonları ile her politikanın uç durumları test edildi (SoC=0, şebeke kesintisi, DR sinyali).

### 5. Teknik Öğrenimler

Kural tabanlı politikaların Optuna ile optimize edilmesi, RL ajanlarının öğrenme eşiğini net biçimde ortaya koydu. Statik eşik tabanlı politikalar, fiyatın dinamik yapısına uyum sağlayamadığı için PPO/SAC gibi RL algoritmalarının ürettiği %30–60 daha yüksek günlük kazancın kaynağını somutlaştırdı.
