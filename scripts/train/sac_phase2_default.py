"""SAC Aşama 2 — varsayılan parametreler (Optuna karşılaştırması için)."""
...
LOG_DIR = _PROJECT_ROOT / "logs" / "sac_phase2_default"
...
def main() -> None:
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(MODEL_DIR / "sac_phase2_default_best"),
    ...
    print("SAC Aşama 2 (DEFAULT) eğitimi başlıyor (300.000 adım)...")
    model.learn(total_timesteps=300_000, callback=eval_callback, progress_bar=True)
    model.save(str(MODEL_DIR / "sac_phase2_default_final"))
    train_env.save(str(MODEL_DIR / "sac_phase2_default_vecnormalize.pkl"))
    print(f"Model kaydedildi: {MODEL_DIR / 'sac_phase2_default_final'}")
