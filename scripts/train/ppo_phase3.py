"""Curriculum Aşama 3 — ertelenebilir yük ile PPO eğitimi.
...
Eğitim sonrası model:
  models/ppo_phase3_final.zip
  models/ppo_phase3_vecnormalize.pkl
"""
...
LOG_DIR   = _PROJECT_ROOT / "logs" / "ppo_phase3"
...
def main() -> None:
    ...
    def make_env_fn():
        return SmartHomeEnergyEnv(
            ...
            enable_deferrable=True,           # Aşama 3
            deferrable_load_power_kw=1.5,
            deferrable_load_hours=1.0,
            deferrable_window=(6, 22),
            deferrable_penalty_coef=2.0,      # PR review sonrası düşürüldü
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(MODEL_DIR / "ppo_phase3_best"),
    ...
    model.save(str(MODEL_DIR / "ppo_phase3_final"))
    train_env.save(str(MODEL_DIR / "ppo_phase3_vecnormalize.pkl"))
    print(f"Model kaydedildi: {MODEL_DIR / 'ppo_phase3_final'}")
