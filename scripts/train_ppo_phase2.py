"""Curriculum Aşama 2 — güneş + ev talebi ile PPO eğitimi."""
...
LOG_DIR = _PROJECT_ROOT / "logs" / "ppo_phase2"
...
def main() -> None:
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=str(MODEL_DIR / "ppo_phase2_best"),
    ...
    print("PPO Aşama 2 eğitimi başlıyor (300.000 adım)...")
    model.learn(total_timesteps=300_000, callback=eval_callback, progress_bar=True)
    model.save(str(MODEL_DIR / "ppo_phase2_final"))
    train_env.save(str(MODEL_DIR / "ppo_phase2_vecnormalize.pkl"))
    print(f"Model kaydedildi: {MODEL_DIR / 'ppo_phase2_final'}")
